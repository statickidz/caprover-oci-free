...
variable "num_worker_instances" {
  description = "Number of CapRover worker instances to deploy."
  type        = number
  default     = 1
}

variable "availability_domain_main" {
  description = "Availability domain for caprover-main instance. For example: AaJg:EU-FRANKFURT-1-AD-1"
  type        = string
}

variable "availability_domain_workers" {
  description = "Availability domain for caprover-worker instances. For example: AaJg:EU-FRANKFURT-1-AD-2"
  type        = string
}
